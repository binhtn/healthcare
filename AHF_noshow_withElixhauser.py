### this dofile proves that elixhauser score is useless in predicting 
...
ahf_patient = pd.read_pickle('ahf_v3.p')

ahf_elix = pd.read_csv('C:/Users/binh/Dropbox (CareSkore)/DataScience/AIDShealth/ahf_with_num_dx.csv', sep=',', header=0)
ahf_elix.columns = map(str.lower, ahf_elix.columns)
ahf_elix =ahf_elix.rename(columns = {"id" : "﻿Id" })
ahf_elix =ahf_elix.rename(columns = {"fldpmppid" : "fldPMPPID" })

ahf_patient['Id']=ahf_patient['﻿Id'].convert_objects(convert_numeric=True)
ahf_elix['Id']=ahf_elix['﻿Id'].convert_objects(convert_numeric=True)

ahf= pd.merge(ahf_patient, ahf_elix , on = ['Id' , 'fldPMPPID'], left_index=True, right_index=True, how="outer")

ahf['ndx_3'] = ahf['num_dx'].apply(lambda x : +1 if x<4 else 0)
ahf['ndx_7'] = ahf['num_dx'].apply(lambda x : +1 if x<11  and x>3 else 0)
ahf['ndx_11'] = ahf['num_dx'].apply(lambda x : +1 if x>10 else 0)



# ahf.to_csv('ahf_ben.csv')
